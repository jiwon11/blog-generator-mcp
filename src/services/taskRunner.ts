...
export async function runFeedbackApplication(
  ...
  try {
    ...
    sendNotification(
      taskId,
      TaskStatus.COMPLETED,
      "피드백이 반영되었습니다. blog_get_status로 결과를 확인하세요."
    );
...
export async function runReviewGeneration(
  ...
  try {
    ...
    sendNotification(
      taskId,
      TaskStatus.COMPLETED,
      `검수가 완료되었습니다. ${result.changes.length}개의 개선사항이 반영되었습니다.`
    );
 ...
 */
export async function runProFeedbackApplication(
  taskId: string,
  feedback: string,
  anthropicApiKey: string
): Promise<void> {
  try {
    ...
    // Claude로 피드백 반영
    const result = await applyFeedbackWithClaude(
      task.result.draft,
      feedback,
      anthropicApiKey
    );
    ...
    sendNotification(
      taskId,
      TaskStatus.COMPLETED,
      "Pro Mode 피드백 반영 완료. blog_get_status로 결과를 확인하세요."
    );
