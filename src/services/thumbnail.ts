...
const THUMBNAIL_PROMPT = (title: string, tags: string[]) =>
  `당신은 기술 블로그의 썸네일/커버 이미지를 찾는 전문가입니다.
 ...
 */
export async function searchThumbnailWithGemini(
  title: string,
  tags: string[],
  apiKey: string
): Promise<string | null> {
 ...
 */
export async function searchThumbnailWithClaude(
  title: string,
  tags: string[],
  apiKey: string
): Promise<string | null> {
 ...
 */
export function injectThumbnailToFrontmatter(draft: string, thumbnailUrl: string): string {
  const frontmatterMatch = draft.match(/^(---\n)([\s\S]*?)(\n---)/);
  if (!frontmatterMatch) return draft;
