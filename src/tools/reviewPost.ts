...
import { ReviewPostInputSchema, ReviewPostInput, ReviewFocus } from "../types.js";

function getReviewPrompt(draft: string, focus: ReviewFocus): string {
...
function parseReviewResponse(response: string): { improved: string; changes: string[] } {
...
Example:
  draft="# React Hooks\\n...", focus="accuracy"`,
    ...
    async (params: ReviewPostInput) => {
      try {
        const focus = (params.focus || ReviewFocus.ALL) as ReviewFocus;
