...
export function registerGenerateDraftTool(server: McpServer): void {
  server.registerTool(
    "blog_generate_draft",
    {
...
Examples:
  ...
  - 개발일지 -> input_type="git_push", content="<git diff output>"`,
    ...
    async (params: GenerateDraftInput) => {
      try {
        const result = await generateBlogDraft(
          params.input_type as InputType,
          params.content,
          (params.style || BlogStyle.TUTORIAL) as BlogStyle,
          (params.language || Language.KO) as Language
        );
