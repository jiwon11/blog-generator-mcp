...
export async function createTask(id, type, input) {
    ...
    db.run(`INSERT INTO tasks (id, type, status, progress, input, result, history, error, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
        task.id,
        task.type,
        task.status,
...
export async function getTask(id) {
    ...
    return {
        ...
        input: JSON.parse(getValue("input")),
        result: getValue("result") ? JSON.parse(getValue("result")) : null,
...
export async function addFeedbackToHistory(id, feedback) {
    ...
    const now = new Date().toISOString();
    db.run(`UPDATE tasks SET history = ?, status = ?, progress = 0, updated_at = ? WHERE id = ?`, [JSON.stringify(newHistory), TaskStatus.PENDING, now, id]);
    await saveDatabase();
}
