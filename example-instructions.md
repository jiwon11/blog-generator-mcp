...
### 웹 참조 자료 활용 (web_search 활성화 시)

> 웹 검색이 활성화된 경우, 외부 자료를 적극 활용하여 글의 설득력과 풍부함을 높이세요.

#### 이미지 링크 활용
- 공식 문서, 기술 블로그의 **아키텍처 다이어그램, 플로우차트, 성능 그래프** 이미지를 검색하여 포함
- `![설명](이미지URL)` 형식으로 본문에 삽입하고, 바로 아래에 출처를 명시
- 각 주요 섹션마다 최소 1개의 외부 시각 자료를 포함하도록 노력

#### 인라인 참조 링크
- 기술 용어/개념 설명 시 공식 문서 또는 권위 있는 블로그로 연결
- `[용어](참조URL)` 형식으로 본문 흐름에 자연스럽게 삽입
- 단순 나열이 아닌, 독자가 더 깊이 학습할 수 있는 맥락에서 제공

#### 참고 자료 분류
글 말미 참고 자료 섹션에서 출처를 분류하여 정리:
- 📚 **공식 문서** — 프레임워크/라이브러리 공식 문서
- 📝 **기술 블로그** — 관련 기술 블로그 글
- 🎓 **튜토리얼·가이드** — 학습용 자료
- 📊 **성능 사례·벤치마크** — 성능 비교 자료
